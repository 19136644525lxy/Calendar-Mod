...
public class StyleManager {
     ...
     *
     * 关键：使用 ResourceManager.getAllResources() 合并所有资源包层级的 styles.json，
    ...
    private static void loadStylesFromJson() {
        ...
        // getAllResources 返回顺序：从低优先级（模组自带）到高优先级（资源包）
        // 所以后来的同名 id 会覆盖先来的
    ...
    private static List<String> loadAllStyleJsons() {
        ...
        try {
            ...
            // getAllResources 返回从低到高优先级的所有资源（等价 Forge 的 getResourceStack）
            List<Resource> stack = MinecraftClient.getInstance().getResourceManager().getAllResources(id);
    ...
    /** 将 assets/namespace/path 格式转为 Identifier */

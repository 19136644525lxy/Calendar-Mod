...
public class CalendarClientConfig {
    ...
    public static Screen buildScreen(Screen parent) {
        ConfigBuilder builder = ConfigBuilder.create()
                ...
                .setTitle(Text.translatable("calendarmod.config.title"));

        ConfigEntryBuilder e = builder.entryBuilder();

        // === HUD 设置分类 ===
        ConfigCategory hudCat = builder.getOrCreateCategory(
                Text.translatable("calendarmod.config.category.hud"));

        hudCat.addEntry(e.startBooleanToggle(
                Text.translatable("calendarmod.config.hud_enabled"),
                hudEnabled)
                .setDefaultValue(true)
                .setSaveConsumer(v -> { hudEnabled = v; save(); })
                .setTooltip(Text.translatable("calendarmod.config.hud_enabled.tooltip"))
                .build());

        hudCat.addEntry(e.startBooleanToggle(
                Text.translatable("calendarmod.config.hud_right_align"),
                hudRightAlign)
                .setDefaultValue(true)
                .setSaveConsumer(v -> { hudRightAlign = v; save(); })
                .setTooltip(Text.translatable("calendarmod.config.hud_right_align.tooltip"))
                .build());

        hudCat.addEntry(e.startIntSlider(
                Text.translatable("calendarmod.config.hud_x"),
                hudX, 0, 500)
                .setDefaultValue(10)
                .setSaveConsumer(v -> { hudX = v; save(); })
                .setTooltip(Text.translatable("calendarmod.config.hud_x.tooltip"))
                .build());

        hudCat.addEntry(e.startIntSlider(
                Text.translatable("calendarmod.config.hud_y"),
                hudY, 0, 500)
                .setDefaultValue(10)
                .setSaveConsumer(v -> { hudY = v; save(); })
                .setTooltip(Text.translatable("calendarmod.config.hud_y.tooltip"))
                .build());

        hudCat.addEntry(e.startBooleanToggle(
                Text.translatable("calendarmod.config.show_today_events"),
                showTodayEvents)
                .setDefaultValue(true)
        ...
        hudCat.addEntry(e.startBooleanToggle(
                Text.translatable("calendarmod.config.show_event_descs"),
        ...
        ConfigCategory screenCat = builder.getOrCreateCategory(
                Text.translatable("calendarmod.config.category.screen"));

        screenCat.addEntry(e.startIntSlider(
                Text.translatable("calendarmod.config.default_month_view"),
        ...
        screenCat.addEntry(e.startBooleanToggle(
                Text.translatable("calendarmod.config.auto_open_login"),
        ...
        // 样式选择下拉框
        {
            ...
            screenCat.addEntry(e.<String>startSelector(
                    Text.translatable("calendarmod.config.style"),
                    ids,
                    displayId)
                    .setNameProvider(id -> {
                        for (StyleManager.StyleInfo s : styles) {
                            if (s.id.equals(id)) return Text.literal(s.name);
                        }
                    ...
                    .setTooltip(Text.translatable("calendarmod.config.style.tooltip"))
                    .build());
        }

 ...
 *
 * <p>26.2 适配：背景绘制由 {@code renderBackground(DrawContext)} 改为
 * {@code extractBackground(GuiGraphicsExtractor,...)}；HtmlScreen 26.2 在
 * {@code extractRenderState} 中已调用 {@code extractBackground} 与 HTML 管线渲染，
 * 故本类无需重写 {@code extractRenderState}。
 */
public class CalendarScreen extends HtmlScreen {
    ...
    /** 26.2：背景绘制改写 extractBackground；淡色毛玻璃遮罩（而非默认全黑遮罩） */
    @Override
     ...
     *
     * <p>26.2：{@code Resource.getInputStream()} → {@code Resource.open()}（可能抛 IOException，由外层 try-catch 吸收）。
     */
    ...
    /** 将 "assets/calendarmod/templates/xxx.css" 转为 Identifier("calendarmod", "templates/xxx.css")（26.2 使用工厂方法） */
    private static Identifier toIdentifier(String path) {
        ...
        try {
            return Identifier.fromNamespaceAndPath(namespace, rest);
        } catch (Exception e) {
            return null;
        }
    }
    ...
    public void handleClick(ClickEvent event) {
        ...
        if ("prev-month".equals(id)) {
            ...
            Minecraft.getInstance().gui.setScreen(CalendarClientConfig.buildScreen(this));
        } else if ("open-calendar-config".equals(id)) {
            Minecraft.getInstance().gui.setScreen(new CalendarConfigScreen());
        } else if ("open-events".equals(id)) {
            Minecraft.getInstance().gui.setScreen(new CalendarEventScreen());
        }
    }
}

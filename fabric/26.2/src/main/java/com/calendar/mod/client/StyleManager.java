 ...
 * 5. 始终保证至少有默认样式可用
 *
 * <p>26.2 适配：Resource 包路径改为 {@code server.packs.resources}，
 * {@code getAllResources} → {@code getResourceStack}，
 * {@code Resource.getInputStream()} → {@code Resource.open()}，
 * Identifier 包路径改为 {@code net.minecraft.resources} 并使用工厂方法构造。
 */
public class StyleManager {
     ...
     *
     * 关键：使用 ResourceManager 合并所有资源包层级的 styles.json，
     * 而不是只取优先级最高的那一个。这样资源包只需提供自己的 styles.json（只包含新增样式），
     * 模组就能自动合并识别，不需要覆盖模组原版的 styles.json。
     *
     * <p>26.2：原 {@code getAllResources(id)} 改为 {@code getResourceStack(id)}，
     * 语义一致——返回从低优先级（模组自带）到高优先级（资源包覆盖/新增）的资源栈。
     */
     ...
     *
     * <p>26.2：{@code getAllResources} → {@code getResourceStack}，
     * {@code Resource.getInputStream()} → {@code Resource.open()}（可能抛 IOException，由外层 try-catch 吸收）。
     */
    private static List<String> loadAllStyleJsons() {
        List<String> results = new ArrayList<>();
        try {
            Identifier id = toIdentifier(STYLES_META_PATH);
            if (id == null) return results;

            // getResourceStack 返回从低到高优先级的所有资源（等价旧 getAllResources）
            List<Resource> stack = Minecraft.getInstance().getResourceManager().getResourceStack(id);
    ...
    /** 将 assets/namespace/path 格式转为 Identifier（26.2 使用 fromNamespaceAndPath 工厂方法） */

...
public class CalendarHud {
    ...
    /**
     * 渲染 HUD。
     * 26.2：渲染上下文由 DrawContext 改为 GuiGraphicsExtractor，
     * 文本绘制 drawText -> text，fill 方法名不变。
     */
    public void render(GuiGraphicsExtractor graphics, int screenWidth, int screenHeight) {
        ...
        for (int layer = SHADOW_BLUR; layer >= 1; layer--) {
            ...
            drawRoundedFill(graphics, sx, sy, sw, sh, RADIUS + layer, col);
        }

        // ========== 2. 半透明主体 + 顶部装饰条（整体圆角） ==========
        drawRoundedFill(graphics, x, y, width, height, RADIUS, hud.body);

        // ========== 3. 顶部高亮条（仅顶部圆角内的一条装饰带） ==========
        int decorH = 3;
        graphics.fill(x + RADIUS, y, x + width - RADIUS, y + decorH, hud.decor);
        fillCornerBand(graphics, x + RADIUS, y + RADIUS, RADIUS, decorH, hud.decor, false, false);
        fillCornerBand(graphics, x + width - RADIUS, y + RADIUS, RADIUS, decorH, hud.decor, true, false);

        // ========== 4. 细边框（比背景深一点，增强立体感） ==========
        drawRoundedBorder(graphics, x, y, width, height, RADIUS, 1, hud.border);
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            graphics.text(font, line, x + padX, ty, color, false);
            ty += lineHeight;
        }
    }
    ...
    private static void fillCornerBand(GuiGraphicsExtractor g, int cx, int cy, int r, int decorH,

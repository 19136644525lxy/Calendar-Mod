...
public class CalendarConfig {
    ...
    public static CalendarConfig load(CompoundTag tag) {
        ...
        if (tag.contains("eraName")) {
            config.eraName = tag.getStringOr("eraName", "");
        }
        if (tag.contains("monthsPerYear")) {
            config.monthsPerYear = Math.max(1, tag.getIntOr("monthsPerYear", 0));
        }
        // 每月天数数组
        if (tag.contains("daysPerMonth")) {
            ListTag list = tag.getListOrEmpty("daysPerMonth");
            config.daysPerMonth = new int[list.size()];
            for (int i = 0; i < list.size(); i++) {
                config.daysPerMonth[i] = Math.max(1, list.getIntOr(i, 0));
            }
        } else {
            // 默认填充
        ...
        // 月份名列表
        if (tag.contains("monthNames")) {
            ListTag list = tag.getListOrEmpty("monthNames");
            config.monthNames = new ArrayList<>();
            for (int i = 0; i < list.size(); i++) {
                config.monthNames.add(list.getStringOr(i, ""));
            }
        }
        if (tag.contains("startYear")) {
            config.startYear = tag.getIntOr("startYear", 0);
        }
        // 星期名列表
        if (tag.contains("weekdayNames")) {
            ListTag list = tag.getListOrEmpty("weekdayNames");
            config.weekdayNames = new ArrayList<>();
            for (int i = 0; i < list.size(); i++) {
                config.weekdayNames.add(list.getStringOr(i, ""));
            }
        }
        return config;
    }
    ...
    public CompoundTag save() {
        ...
        for (int days : daysPerMonth) {
            daysList.add(IntTag.valueOf(days));
        }

...
public class CalendarEvent {
    ...
    public static CalendarEvent load(CompoundTag tag) {
        try {
            ...
            String description = "";
            if (tag.contains("description", Tag.TAG_STRING)) {
                description = tag.getString("description");
            }

            int color = tag.contains("color", Tag.TAG_INT) ? tag.getInt("color") : 0xFFFFFFFF;

            boolean isFixed = tag.contains("isFixed", Tag.TAG_BYTE) && tag.getBoolean("isFixed");

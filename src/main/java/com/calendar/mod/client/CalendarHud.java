...
public class CalendarHud {
    ...
    public boolean isMouseOver(int mouseX, int mouseY, int screenWidth, int screenHeight) {
        ...
        int[] size = computeHudSize(lines, mc.font);
    ...
    public void render(GuiGraphics guiGraphics, int screenWidth, int screenHeight) {
        ...
        for (int layer = SHADOW_BLUR; layer >= 1; layer--) {
            ...
            drawRoundedFill(guiGraphics, sx, sy, sw, sh, RADIUS + layer, col);
        }

        // ========== 2. 半透明主体 + 顶部装饰条（整体圆角） ==========
        drawRoundedFill(guiGraphics, x, y, width, height, RADIUS, hud.body);

        // ========== 3. 顶部高亮条（仅顶部圆角内的一条装饰带） ==========
        // 顶部 3px 一条稍亮颜色，提升层次感
        int decorH = 3;
        // 顶部矩形 + 只在上面两角做圆角（通过 clip 近似：直接画圆角后的矩形上半）
        guiGraphics.fill(x + RADIUS, y, x + width - RADIUS, y + decorH, hud.decor);
        // 左侧顶部圆角内的装饰（画半圆近似）
        fillCornerBand(guiGraphics, x + RADIUS, y + RADIUS, RADIUS, decorH, hud.decor, false, false);
        fillCornerBand(guiGraphics, x + width - RADIUS, y + RADIUS, RADIUS, decorH, hud.decor, true, false);

        // ========== 4. 细边框（比背景深一点，增强立体感） ==========
        drawRoundedBorder(guiGraphics, x, y, width, height, RADIUS, 1, hud.border);
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            guiGraphics.drawString(font, line, x + padX, ty, color, false);
            ty += lineHeight;
        }
    }

    /** 计算 HUD 尺寸（宽度、高度），保证渲染与命中测试一致 */
    private int[] computeHudSize(List<String> lines, Font font) {
        int padX = 12;
        int padY = 8;
        int decorH = 3;
        int lineHeight = font.lineHeight + 4;
    ...
    private static void fillCornerBand(GuiGraphics g, int cx, int cy, int r, int decorH,
    ...
    private List<String> buildLines() {
        ...
        lines.add(cfg.getEraName() + " " + today.getYear() + " "
                + Language.getInstance().getOrDefault("calendarmod.label.year")

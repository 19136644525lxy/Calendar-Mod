...
public class CalendarScreen extends HtmlScreen {
    ...
    public CalendarScreen() {
        super(Component.translatable("calendarmod.screen.title"));
    ...
    /** 重写 render：先画淡色遮罩（而非全黑背景），再执行 HTML 管线渲染 */
    @Override
    public void render(net.minecraft.client.gui.GuiGraphics graphics,
                       int mouseX, int mouseY, float partialTick) {
        // 取消默认 renderBackground 全黑遮罩，改为淡色毛玻璃质感
        if (this.minecraft != null && this.minecraft.level != null) {
            graphics.fill(0, 0, this.width, this.height, 0x25FFFFFF);
        } else {
            graphics.fill(0, 0, this.width, this.height, 0xF0F2F5FF);
        }
        // 执行 HTML 管线渲染（带居中偏移）
        if (getPipeline() != null && getRootLayout() != null) {
            graphics.pose().pushPose();
            graphics.pose().translate(getLayoutOffsetX(), getLayoutOffsetY(), 0);
            getPipeline().render(graphics, getRootLayout());
            graphics.pose().popPose();
        }
        // Screen.render 最后一步：渲染所有注册的 renderables
        for (var w0 : this.renderables) {
            w0.render(graphics, mouseX, mouseY, partialTick);
        }
    }

    private long getWorldDay() {
        if (minecraft != null && minecraft.level != null) {
            return minecraft.level.getDayTime() / 24000L;
        }
        return 0L;
    }
    ...
    /** 将 "assets/calendarmod/templates/xxx.css" 转为 "calendarmod:templates/xxx.css" */
    private static ResourceLocation toResourceLocation(String path) {
        // 期望格式：assets/<namespace>/<rest>
    ...
    protected String getCss() {
        ...
        int rootPad = Math.max(6, screenH / 30);          // 根容器内边距
        int gap = Math.max(4, screenH / 50);               // 元素间距
        int headerPad = Math.max(6, screenH / 35);         // 标题区内边距
        int navBtnSize = Math.max(28, screenH / 12);       // 导航按钮尺寸
        int titleFont = Math.max(14, screenH / 22);        // 标题字号
        int eraFont = Math.max(9, screenH / 36);           // 纪元字号
        int subtitleFont = Math.max(9, screenH / 36);      // 副标题字号
        int weekCellPad = Math.max(3, screenH / 60);       // 星期格内边距
        int weekFont = Math.max(10, screenH / 30);         // 星期字号
        int dayGap = Math.max(3, screenH / 60);            // 日期格间距

        // 日期格子高度：屏幕高度减去其他区域后平均分给 6 行
        int btnHeight = Math.max(18, screenH / 16);                        // 底部按钮高度（缩小）
        int fixedH = rootPad * 2 + (navBtnSize + headerPad * 2) + gap * 4  // 根padding+标题+间距
                + (weekCellPad * 2 + weekFont + 2)                        // 星期行
                + btnHeight + gap;                                        // 底部按钮+间距
        int eventsPanelH = Math.max(50, screenH / 5);                      // 事件面板高度
        fixedH += eventsPanelH + gap;
        int dayH = Math.max(20, (screenH - fixedH) / 6);                   // 日期格高度
        int dayPad = Math.max(2, dayH / 10);
        int dayNumFont = Math.max(11, dayH / 2);                           // 日期数字字号
        int dayEventFont = Math.max(8, dayH / 4);                          // 日期事件文字字号
        int btnWidth = Math.max(50, (screenW - rootPad * 2 - gap * 2) / 3);
        int btnFont = Math.max(9, screenH / 32);                          // 底部按钮字号（缩小）
        int eventRowPad = Math.max(3, eventsPanelH / 20);                  // 事件行内边距
        int eventFont = Math.max(10, eventsPanelH / 14);                   // 事件名字字号
        int eventDescFont = Math.max(8, eventsPanelH / 18);                // 事件描述字号
    ...
    public void handleClick(ClickEvent event) {
        ...
        if ("prev-month".equals(id)) {
            ...
            Minecraft.getInstance().setScreen(CalendarClientConfig.buildScreen(this));
        } else if ("open-calendar-config".equals(id)) {
            Minecraft.getInstance().setScreen(new CalendarConfigScreen());
        } else if ("open-events".equals(id)) {
            Minecraft.getInstance().setScreen(new CalendarEventScreen());
        }
    }
}